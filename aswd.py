...
class ModernEmployeeScheduler:
    ...
    def generate_shift_types_list(self):
        ...
        shift_types_list = []
        for shift_name, settings in self.shift_types_data.items():
            if settings["start"] == "00:00" and settings["end"] == "00:00":
                shift_types_list.append(shift_name)
            else:
                shift_types_list.append(f"{shift_name} ({settings['start']}-{settings['end']})")
        return shift_types_list
    
    def setup_drag_and_drop(self):
        """Настройка перетаскивания смен"""
        # Привязываем события мыши к таблице с графиком
        self.schedule_tree.bind('<ButtonPress-1>', self.on_drag_start)
        self.schedule_tree.bind('<B1-Motion>', self.on_drag_motion)
        self.schedule_tree.bind('<ButtonRelease-1>', self.on_drag_stop)
        self.schedule_tree.bind('<Enter>', lambda e: self.schedule_tree.config(cursor='hand2'))
        self.schedule_tree.bind('<Leave>', lambda e: self.schedule_tree.config(cursor=''))
        
    def on_drag_start(self, event):
        """Начало перетаскивания смены"""
        region = self.schedule_tree.identify("region", event.x, event.y)
        if region == "cell":
            # Получаем информацию о ячейке
            row = self.schedule_tree.identify_row(event.y)
            column = self.schedule_tree.identify_column(event.x)
            
            if row and column:
                # Получаем номер дня из колонки
                try:
                    day = int(column.replace('#', '')) - 1  # -1 потому что первая колонка - имя работника
                    if day > 0:  # Пропускаем колонку с именем
                        # Получаем имя работника
                        employee = self.schedule_tree.item(row)['values'][0]
                        
                        # Проверяем, есть ли смена в этой ячейке
                        shift_info = self.schedule_tree.item(row)['values'][day]
                        
                        if shift_info and shift_info.strip():
                            ...
                            self.dragging = True
                            self.schedule_tree.config(cursor='fleur')
                            
                            # Выделяем ячейку
                            self.schedule_tree.selection_set(row)
    ...
    def on_drag_stop(self, event):
        ...
        if self.dragging and self.drag_data["item"]:
            ...
            if row and column:
                try:
                    target_day = int(column.replace('#', '')) - 1
                    if target_day > 0:  # Пропускаем колонку с именем
                        # Получаем целевого работника
                        target_employee = self.schedule_tree.item(row)['values'][0]
                        
                        # Проверяем, что перетащили на другую ячейку
                        if (self.drag_data["employee"] != target_employee or 
                            self.drag_data["day"] != target_day):
                            
                            # Получаем информацию о смене
                            shift_info = self.drag_data["shift_info"]
                            
                            # Парсим информацию о смене
                            shift_type = shift_info.split('\n')[0] if '\n' in shift_info else shift_info
                            
                            # Получаем время из смены (если есть)
                            start_time = "08:00"
                            end_time = "16:00"
                            if '(' in shift_type:
                                # Если есть время в формате "Тип (08:00-16:00)"
                                shift_type_name = shift_type.split('(')[0].strip()
                                if ')' in shift_type:
                                    time_part = shift_type.split('(')[1].split(')')[0]
                                    if '-' in time_part:
                                        start_time, end_time = time_part.split('-')
                            elif shift_type in self.shift_types_data:
                                # Если это базовый тип смены
                                shift_type_name = shift_type
                                start_time = self.shift_types_data[shift_type_name]["start"]
                                end_time = self.shift_types_data[shift_type_name]["end"]
                            else:
                                # По умолчанию
                                shift_type_name = shift_type
                            
                            # Находим ID целевого работника
                            target_employee_obj = next((emp for emp in self.employees 
                                                      if emp["name"] == target_employee), None)
                            ...
                            if target_employee_obj:
                                ...
                                try:
                                    ...
                                    # Сначала удаляем смену у исходного работника (если это был другой работник)
                                    if self.drag_data["employee"] != target_employee:
                                        # Находим ID исходного работника
                                        source_employee = next((emp for emp in self.employees 
                                                              if emp["name"] == self.drag_data["employee"]), None)
                                        if source_employee:
                                            # Удаляем смену у исходного работника
                                            source_date = date(self.current_year, self.current_month, self.drag_data["day"])
                                            source_date_str = source_date.strftime("%Y-%m-%d")
                                            self.shifts = [s for s in self.shifts 
                                                         if not (s["employee_id"] == source_employee["id"] and 
                                                                 s["date"] == source_date_str)]
                                    
                                    # Удаляем существующую смену у целевого работника
                                    self.shifts = [s for s in self.shifts 
                                                 if not (s["employee_id"] == target_employee_obj["id"] and 
                                                         s["date"] == date_str)]
                                    
                                    # Создаем строку для отображения
                                    if start_time == "00:00" and end_time == "00:00":
                                        display_shift = shift_type_name
                                    else:
                                        display_shift = f"{shift_type_name}\n{start_time}-{end_time}"
                                    
                                    # Формируем полное название смены для хранения
                                    if start_time == "00:00" and end_time == "00:00":
                                        full_shift_name = shift_type_name
                                    else:
                                        full_shift_name = f"{shift_type_name} ({start_time}-{end_time})"
                                    
                                    # Добавляем новую смену
                                    self.shifts.append({
                                        "employee_id": target_employee_obj["id"],
                                        "employee_name": target_employee,
                                        "date": date_str,
                                        "shift": full_shift_name,
                                    ...
                                    self.status_bar.config(text=f"Смена перемещена на {target_date.strftime('%d.%m.%Y')} для {target_employee}")
                                    
                                except ValueError as e:
                                    messagebox.showwarning("Ошибка", f"Неверная дата: {str(e)}", parent=self.root)
                except (ValueError, IndexError):
                    pass
        ...
        self.dragging = False
        self.drag_data = {"item": None, "day": None, "employee": None}
        self.schedule_tree.config(cursor='')
    ...
    # ==================== НОВЫЕ МЕТОДЫ ДЛЯ ТИПОВ СМЕН ====================
    
    def choose_new_shift_color(self):
        """Выбор цвета для нового типа смены"""
        color = colorchooser.askcolor(title="Выберите цвет для типа смены", 
                                      initialcolor=self.new_shift_color.get(),
                                      parent=self.root)
        if color[1]:
            self.new_shift_color.set(color[1])
    
    def add_shift_type(self):
        """Добавление нового типа смены"""
        name = self.new_shift_name.get().strip()
        start_hour = self.new_start_hour.get()
        start_minute = self.new_start_minute.get()
        end_hour = self.new_end_hour.get()
        end_minute = self.new_end_minute.get()
    ...
    def delete_shift_type(self):
        """Удаление выбранного типа смены"""
        ...
        shift_name = item['values'][0]
        
        # Нельзя удалить базовые типы смен
        protected_shifts = ["Утренняя", "Дневная", "Вечерняя", "Ночная", "Выходной", "Сокращенная"]
        if shift_name in protected_shifts:
            messagebox.showwarning("Внимание", f"Тип смены '{shift_name}' является базовым и не может быть удален", 
                                 parent=self.root)
            return
        
        if messagebox.askyesno("Подтверждение", 
                              f"Удалить тип смены '{shift_name}'?\nВсе смены этого типа будут удалены.", 
                              parent=self.root):
            # Удаляем тип смены
            del self.shift_types_data[shift_name]
            
            # Удаляем все смены этого типа
            self.shifts = [s for s in self.shifts if s.get("shift_type") != shift_name]
            ...
            messagebox.showinfo("Успех", f"Тип смены '{shift_name}' удален", parent=self.root)
    
    def edit_shift_type(self):
        """Редактирование выбранного типа смены"""
        selection = self.shift_types_tree.selection()
        if not selection:
        ...
        edit_window.title(f"Редактирование типа смены: {shift_name}")
        ...
        settings = self.shift_types_data[shift_name]
        start_hour, start_minute = settings["start"].split(":")
        end_hour, end_minute = settings["end"].split(":")
        color = settings["color"]
        
        # Название смены
        ttk.Label(main_frame, text="Название:", font=('Segoe UI', 11)).pack(anchor=tk.W, pady=(0, 5))
        name_var = tk.StringVar(value=shift_name)
        name_entry = ttk.Entry(main_frame, textvariable=name_var, font=('Segoe UI', 11))
        name_entry.pack(fill=tk.X, pady=(0, 10))
        
        # Время начала
        ttk.Label(main_frame, text="Начало:", font=('Segoe UI', 11)).pack(anchor=tk.W, pady=(0, 5))
        time_frame = ttk.Frame(main_frame)
        time_frame.pack(fill=tk.X, pady=(0, 10))
        
        start_hour_var = tk.StringVar(value=start_hour)
        ttk.Combobox(time_frame, textvariable=start_hour_var, 
                    values=[f"{i:02d}" for i in range(0, 24)], 
                    width=3, state="readonly").pack(side=tk.LEFT)
        ttk.Label(time_frame, text=":").pack(side=tk.LEFT)
        
        start_minute_var = tk.StringVar(value=start_minute)
        ttk.Combobox(time_frame, textvariable=start_minute_var, 
                    values=[f"{i:02d}" for i in range(0, 60, 5)], 
                    width=3, state="readonly").pack(side=tk.LEFT)
        
        ttk.Label(time_frame, text="до").pack(side=tk.LEFT, padx=10)
        
        # Время окончания
        end_hour_var = tk.StringVar(value=end_hour)
        ttk.Combobox(time_frame, textvariable=end_hour_var, 
                    values=[f"{i:02d}" for i in range(0, 24)], 
                    width=3, state="readonly").pack(side=tk.LEFT)
        ttk.Label(time_frame, text=":").pack(side=tk.LEFT)
        
        end_minute_var = tk.StringVar(value=end_minute)
        ttk.Combobox(time_frame, textvariable=end_minute_var, 
                    values=[f"{i:02d}" for i in range(0, 60, 5)], 
                    width=3, state="readonly").pack(side=tk.LEFT)
        
        # Цвет
        ttk.Label(main_frame, text="Цвет:", font=('Segoe UI', 11)).pack(anchor=tk.W, pady=(0, 5))
        color_frame = ttk.Frame(main_frame)
        color_frame.pack(fill=tk.X, pady=(0, 10))
        
        color_var = tk.StringVar(value=color)
        ttk.Entry(color_frame, textvariable=color_var, width=10, 
                 font=('Segoe UI', 11)).pack(side=tk.LEFT, padx=(0, 5))
        
        def choose_color():
            chosen_color = colorchooser.askcolor(title="Выберите цвет", 
        ...
        ttk.Button(color_frame, text="🎨 Выбрать", 
                  command=choose_color, style='Primary.TButton').pack(side=tk.LEFT)
        ...
        def save_changes():
            ...
            # Если имя изменилось
            if new_name != shift_name:
                if new_name in self.shift_types_data:
                    messagebox.showwarning("Внимание", "Тип смены с таким названием уже существует", 
                                         parent=edit_window)
                    return
                ...
                # Обновляем смены с старым типом
                for shift in self.shifts:
                    if shift.get("shift_type") == shift_name:
                        shift["shift_type"] = new_name
                        if shift["start_time"] == "00:00" and shift["end_time"] == "00:00":
                            shift["shift"] = new_name
                        else:
                            shift["shift"] = f"{new_name} ({shift['start_time']}-{shift['end_time']})"
    ...
    def on_shift_type_selected(self, event=None):
        ...
        if shift_type:
            ...
            if base_name in self.shift_types_data:
                start_time = self.shift_types_data[base_name]["start"]
                end_time = self.shift_types_data[base_name]["end"]
        ...
        if messagebox.askyesno("Подтверждение", 
            ...
            for day in range(1, num_days + 1):
                ...
                for emp in self.employees:
                    ...
                    start_time = self.shift_types_data.get(shift_type, {}).get("start", "08:00")
                    end_time = self.shift_types_data.get(shift_type, {}).get("end", "16:00")
    ...
    def choose_color(self, shift_name, current_color):
        ...
        color = colorchooser.askcolor(title=f"Выберите цвет для {shift_name}", 
                                      ...
                                      parent=self.root)
        if color[1]:
            self.shift_types_data[shift_name]['color'] = color[1]
            self.update_schedule_display()
